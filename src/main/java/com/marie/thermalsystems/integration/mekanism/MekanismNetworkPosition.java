...
/**
 * Represents a point on a Mekanism thermodynamic conductor network. Unlike
 * {@link MekanismBlockHeatSource}, which adapts a single Mekanism machine's
 * own heat state, this sums {@code getHeatOutput()}/{@code getCoolingOutput()}
 * across every Thermal-Systems-capable block reachable on the network this
 * position belongs to, discovered via {@link MekanismNetworkDiscovery}.
 *
 ...
 * tracking every cable in the world. Every entry is keyed by the pair of
 ...
 * wired to {@code ServerStoppingEvent} in {@link MekanismIntegration#init}, so
 ...
 */
final class MekanismNetworkPosition implements IHeatSource, ICoolingSource {
    ...
    MekanismNetworkPosition(Level level, BlockPos pos) {
        this.level = level;
    ...
    private CacheEntry resolve() {
        ...
        int interval = ThermalConfig.MEKANISM_NETWORK_RECOMPUTE_INTERVAL.get();
    ...
    private CacheEntry recompute() {
        ...
        return new CacheEntry(level.getGameTime(), MekanismNetworkSum.sum(heatOutputs), MekanismNetworkSum.sum(coolingOutputs));
    }
